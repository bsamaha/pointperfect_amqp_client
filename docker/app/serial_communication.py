...
class SerialCommunication:
    def __init__(self, port, baudrate, timeout, device_id, amqp_client, gnss_messages):
        self.stream = None
    ...
    async def read_and_send_data(self):
        while True:
            if self.stream and self.stream.in_waiting:
                data_dict = MessageProcessor.process_data(stream=self.stream, device_id=self.device_id, gnss_messages=self.gnss_messages)
                if data_dict:
...
class MessageProcessor:
    @staticmethod
    def process_data(stream, device_id, gnss_messages):
        try:
